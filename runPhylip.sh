...
#Run seqboot
for i in *.phy; do cp $i infile; $phylipDir/seqboot${phylipSuffix}/seqboot < $scriptsDir/seqbootCMD > $(echo $i | sed "s/.phy/_seqboot.log/g"); cp outfile $(echo $i | sed "s/.phy/_bootstrap.phy/");done

#Run dollop for all bootstrapped sequences
for i in *bootstrap.phy; do cp $i infile; $phylipDir/dollop${phylipSuffix}/dollop < $scriptsDir/dollopCMDBoost > $(echo $i | sed "s/.phy/_dollop.log/g"); cp outtree $(echo $i | sed "s/.phy/_bootstrapDollop.trees/");done

#Run dollop for the full datasets
for i in $(ls *.phy | grep -v bootstrap); do cp $i infile; $phylipDir/dollop${phylipSuffix}/dollop < $scriptsDir/dollopCMD > $(echo $i | sed "s/.phy/_dollopFull.log/g"); cp outtree $(echo $i | sed "s/.phy/_dollopFull.trees/");done
